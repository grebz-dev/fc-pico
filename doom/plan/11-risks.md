...
| R12 | Audio arrangement quality is poor with `--auto`. | High | Sounds bad | Human-tuned FamiStudio projects for the most-heard tracks (E1M1, intro, intermission) | listening session |
...
| Q3 | Does the engine's host build work with pico-sdk's `host` platform plus a shim, without SDL? | P0-T3 | 08 |
| Q4 | Mesen2 pinned commit: does `MapperReadVram` see rendering reads with a distinguishing `MemoryOperationType`? Which Lua input API? | P0-T11 | 09 |
...
| Q10 | Does Mesen2 emulate the DMC DMA / controller-read conflict? (Believed yes.) | P4-T5 | 09 |
