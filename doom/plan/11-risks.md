...
|----|------|-----------|--------|-----------------------|----------------|
| R1 | The PPU-bus model cannot be calibrated because the count discrepancy in [01](01-constraints.md) has an explanation the trace does not reveal (e.g. board-level CS1 gating). | Medium | Model goldens are regression-only; co-sim may pass while hardware fails | Keep L7 checks per milestone; make the mapper's `cs1_mask` and per-line read counts data-driven from the trace rather than derived. The stream layout is fixed regardless (01, prior art). Fallback sync source: vblank-gap detection as PiPU's FX2 does (09). | [P0-T10](10-workplan.md#p0-t10-model-calibration) cannot fit the trace |
...
| R3 | Frame rate below 15 fps on RP2350 at 150 MHz with the added conversion. | Low-Medium | Unplayable feel | Overclock ([P5-T1](10-workplan.md#p5-t1-overclock-evaluation----spec-01-rp2040-doomsrci_mainc-size-m-depends-p1-t9-steps-200250270-mhz-with-vreg-and-qmi-timing-pio-bus-validation-trace--stats-at-each-fps-gain-table-acceptance-chosen-clock-documented-hw-1-h-soak-at-that-clock), precedent 270-276 MHz); converter optimisation; reduce `RENDER_COL_MAX`; drop `NO_DRAW_*` features | [P1-T8](10-workplan.md#p1-t8-hardware-session-m1-hw) fps < 15 |
| R4 | 4 MB flash assumption wrong (smaller). | Low | WHX does not fit | Measure at [P0-T9](10-workplan.md#p0-t9-hardware-trace-capture-hw); if 2 MB, fall back to RP2040 Doom's super-tiny layout (`0x10040000`) and a 256 KB firmware cap | `flash_get_size()` |
...
| R8 | Mesen2 build in CI too slow or its Lua API differs from the plan. | Medium | Co-sim delayed | Cache the build by commit; verify API names at [P0-T11](10-workplan.md#p0-t11-mesen2-co-simulation-skeleton) against `LuaDocumentation.json` of the pinned commit; keep scenarios small | [P0-T11](10-workplan.md#p0-t11-mesen2-co-simulation-skeleton) > 20 min per run |
...
| R14 | `pico-sdk` host platform cannot build the engine without `pico_host_sdl`. | Medium | No host runs / no co-sim | Grow `sim/host_shim/` (it is a few hundred lines: multicore via pthreads, sems, alarms) | [P0-T3](10-workplan.md#p0-t3-engine-host-build-without-sdl) |
...
|----|----------|-------|-----------------------|
| Q1 | Exact per-line qualifying-read count and CS1 decode (see [01](01-constraints.md)). | [P0-T9](10-workplan.md#p0-t9-hardware-trace-capture-hw)/T10 | 01, `sim/ppubus` defaults |
| Q2 | Flash size and part; does `flash_get_size()` (SFDP) work on it? | [P0-T9](10-workplan.md#p0-t9-hardware-trace-capture-hw) | 01, 08 |
| Q3 | Does the engine's host build work with pico-sdk's `host` platform plus a shim, without SDL? Partly answered: the base host platform has no `pico_multicore` implementation (headers only) and no alarms; `sim/host_shim/` supplies them (08). Remaining: confirm by linking. | [P0-T3](10-workplan.md#p0-t3-engine-host-build-without-sdl) | 08 |
| Q4 | Mesen2 pinned commit: does `MapperReadVram` see rendering reads with a distinguishing `MemoryOperationType`? (Lua API names are now recorded in [09](09-testing-ci.md): `setInput`, `read` with `nesPpuDebug`, `addMemoryCallback`, `stop`.) | [P0-T11](10-workplan.md#p0-t11-mesen2-co-simulation-skeleton) | 09 |
...
| Q6 | nesdev facts assumed in [01](01-constraints.md) (2-dot access, `/RD` width, pre-render fetches): confirm against the wiki and the trace. | [P0-T10](10-workplan.md#p0-t10-model-calibration) | 01 |
...
| Q8 | Is `DEMO1_ONLY` needed for size on RP2350 with the fcpico target? | [P0-T2](10-workplan.md#p0-t2-engine-fork-superbuild-guard-and-empty-fcpico-platform) | 08 |
| Q9 | Does the engine's `PICO_RP2350` path already move the `stbar` cache out of scratch X, and is scratch X free for a 4 KB core 1 stack? | [P1-T3](10-workplan.md#p1-t3-device-integration-cores-irqs-semaphores) | 01, 02 |
...
| Q12 | Which two-tile pair per line is uncounted by `fcppu_rna` (prefetch or off-screen), and is it the same on every console? | [P0-T9](10-workplan.md#p0-t9-hardware-trace-capture-hw) | 01 |
